(function () {
    ...
    SKU.prototype.init = function () {
        ...
        _this.warp.find('.sku_attr_select .btn').click(function () {
            let _dom = $(this);
            if (!_dom.hasClass('btn-primary')) {
                _dom.addClass('btn-primary').removeClass('btn-default')
        ...
        _this.warp.find('.sku_attr_key_val').on('click', '.Js_add_attr_val', function () {
            let html = '<div class="sku_attr_val_item">' +
                ...
                '</div>' +
                '<span class="btn btn-white Js_remove_attr_val">-<i class="glyphicon glyphicon-remove"></i></span>' +
                '</div>';
            $(this).before(html);
        });
        ...
        _this.warp.find('.Js_add_attr_name').click(function () {
            let html = '<tr>' +
                ...
                '</td>' +
                '<td>' +
                '<span class="btn btn-white Js_remove_attr_name">移除</span>' +
                '</td>' +
                '</tr>';
        ...
        // 统一零售价
        _this.warp.find('.sku_edit_warp thead').on('keyup', 'input.Js_pre_price', function () {
            _this.commonPrePrice = $(this).val();
            _this.warp.find('.sku_edit_warp tbody td[data-field="pre_price"] input').val(_this.commonPrePrice);
            _this.processSku()
        });
    ...
    SKU.prototype.SKUForm = function (default_sku) {
        ...
        if (attr_names.length === 0) {
            ...
            // thead_html += '<th style="width: 100px">规格编号</th>';
            thead_html += '<th style="width: 100px">价格 <input value="' + _this.commonPrice + '" type="text" style="width: 50px" class="Js_price"></th>';
            thead_html += '<th style="width: 100px">零售价 <input value="' + _this.commonPrePrice + '" type="text" style="width: 50px" class="Js_pre_price"></th>';
            ...
            // 根据计算的笛卡尔积渲染tbody
            let tbody_html = '';
            cartesianProductOf.forEach(function (sku_item,index_1) {
                tbody_html += '<tr>';
                ...
                // tbody_html += '<td data-field="pic"><input value="' + _this.commonCode + '" type="text" class="form-control"></td>';
                tbody_html += '<td data-field="id" style="display: none;"><input value="' + (index_1+1) + '" type="hidden"></td>';
                tbody_html += '<td data-field="price"><input value="' + _this.commonPrice + '" type="text" class="form-control"></td>';
                tbody_html += '<td data-field="pre_price"><input value="' + _this.commonPrePrice + '" type="text" class="form-control"></td>';
    ...
    SKU.prototype.processSku = function () {
        let _this = this;
        let sku_json = {};
        sku_json.type = _this.warp.find('.sku_attr_select .btn.btn-primary').attr('data-type');
